---
title: "Missing Data: Methods and Treatment"
author: "Shanny Foo"
date: "10/06/2022"
...
GENFI <- GENFI %>%
  ...
  mutate(TIV_med = median(c(TIV_mm3[Visit=="1"], TIV_mm3[Visit=="2"], TIV_mm3[Visit=="3"], TIV_mm3[Visit=="11"], TIV_mm3[Visit=="12"], TIV_mm3[Visit=="13"], TIV_mm3[Visit=="14"], TIV_mm3[Visit=="15"]), na.rm=TRUE))
...
## GENFI2 Data: For analyses including variables only collected in GENFI2
GENFI2 <- GENFI %>%
  filter(Cohort=="GENFI2")
...
colSums(is.na(GENFI2))


## Calculate sum of missing data by row
sum(is.na(GENFI2$MiniSEA))
```



--- MISSING DATA TREATMENT: DELETION
...
# Example
lm(MiniSEA ~ ClinicalStatus + GeneticMutation, data=GENFI_numeric, na.action=na.omit)
...
# Example
impute(GENFI_numeric$MiniSEA, mean)  # replace with mean
impute(GENFI_numeric$MiniSEA, median)  # median
impute(GENFI_numeric$MiniSEA, 20)  # replace specific number
# or if you want to impute manually
GENFI_numeric$MiniSEA[is.na(GENFI_numeric$MiniSEA)] <- mean(GENFI_numeric$MiniSEA, na.rm = T)  # not run

# Lets compute the accuracy when it is imputed with mean
library(DMwR)
actuals <- original$MiniSEA[is.na(GENFI_numeric$MiniSEA)]
predicteds <- rep(mean(GENFI_numeric$MiniSEA, na.rm=T), length(actuals))
...
--- MISSING DATA TREATMENT: PREDICTION MODEL
...
GENFI2_numeric <- GENFI2 %>%
  select(Cohort, Age, Education, Gender, MiniSEA, RSMS, Ekman, IRI, FauxPas, TIV, Total_Brain, aINS)%>%
  filter(Cohort=="GENFI2")

summary(GENFI2_numeric)

md.pattern(GENFI2_numeric)
...
mice_plot <- aggr(GENFI2_numeric, col=c('navyblue','yellow'),
                  numbers=TRUE, sortVars=TRUE,
                  labels=names(GENFI2_numeric), cex.axis=.7,
...
imputed_Data <- mice(GENFI2_numeric, m=5, maxit = 50, method = 'pmm', seed = 500)
...
imputed_Data$imp$MiniSEA  # check imputed values
...
fit <- with(data = GENFI2_numeric, exp = lm(MiniSEA ~ RSMS + aINS))  # build predictive model
...
GENFI2_numeric <- GENFI2 %>%
  ungroup() %>%
  filter(Cohort=="GENFI2") %>%
  select(ClinicalStatus, GeneticMutation, Age, Education, Gender, MiniSEA, RSMS, Ekman, IRI, FauxPas, TIV, Total_Brain, aINS)
...
amelia_fit <- amelia(GENFI2_numeric, m=5, parallel = "multicore", noms = c("ClinicalStatus", "GeneticMutation"))
...
amelia_fit$imputations[[5]]$MiniSEA
...
GENFI2_numeric.imp <- missForest(GENFI2_numeric.mis)
...
GENFI2_numeric.imp$ximp

# Check imputation error
GENFI2_numeric.imp$OOBerror
...
GENFI2_numeric.err <- mixError(GENFI2_numeric.imp$ximp, GENFI2_numeric.mis, GENFI2_numeric)
GENFI2_numeric.err
...
impute_arg <- aregImpute(~ MiniSEA + aINS + Age + ClinicalStatus + GeneticMutation, data = GENFI2_numeric, n.impute = 5)  

# Check output: The output shows R² values for predicted missing values. Higher the value, better are the values predicted. 
impute_arg

# Check imputed values
impute_arg$imputed$MiniSEA  # check imputed variable MiniSEA
...
GENFI2_numeric$imputed_MiniSEA <- with(GENFI2_numeric, impute(MiniSEA, mean))  # similarly you can use min, max, median to impute missing value

# Impute with random value
GENFI2_numeric$imputed_MiniSEA2 <- with(GENFI2_numeric, impute(MiniSEA, 'random'))
...
mi_data <- mi(GENFI2_numeric, seed = 335)  # imputing missing value with mi
...
```{r}
# MiniSEA
knnOutput <- knnImputation(GENFI2_Clean[, !names(GENFI2_Clean) %in% "MiniSEA"])  # perform knn imputation.
anyNA(knnOutput)

# Lets compute the accuracy.
actuals <- original$ClinicalStatus[is.na(GENFI2_Clean$ClinicalStatus)]
predicteds <- knnOutput[is.na(GENFI2_Clean$ClinicalStatus), "ClinicalStatus"]
regr.eval(actuals, predicteds)
